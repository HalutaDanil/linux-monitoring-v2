...
check_avail_size() {
  local avail_size
  avail_size=$(df -h / | awk 'NR==2 {print $4}')

  if [[ "$avail_size" != *G && "$avail_size" != *T ]]; then
...
check_dir() {
  ...
  if [[ "$path" == */bin* || "$path" == */sbin* ]]; then
    printf "Путь не должен содержать bin или sbin\n" >&2
    return 1
  fi
}
...
check_size() {
  local size="$1"
  if [[ ! "$size" =~ ^[0-9]+$ ]]; then
    printf "Значение в размере должно состоять только из цифр и букв Mb, обозначающих мегабайты\n" >&2
    return 1
